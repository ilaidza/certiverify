...
import '../services/api_service.dart';
import '../utils/theme.dart';
import 'graduate_credential_detail_screen.dart';

class GraduateCredentialsScreen extends StatefulWidget {
  const GraduateCredentialsScreen({super.key});

  @override
  State<GraduateCredentialsScreen> createState() => _GraduateCredentialsScreenState();
}

class _GraduateCredentialsScreenState extends State<GraduateCredentialsScreen>
    with SingleTickerProviderStateMixin {
  final ApiService _apiService = ApiService();
  ...
  Future<void> _fetchCredentials() async {
    ...
    setState(() {
      ...
      if (result['success']) {
        _credentials = List<Map<String, dynamic>>.from(result['credentials']);
  ...
  Widget build(BuildContext context) {
    ...
    final activeCount = _credentials.where((c) => c['status'] == 'active').length;
    final revokedCount = _credentials.where((c) => c['status'] == 'revoked').length;
            ...
            : _error != null
                ...
                : _filteredCredentials.isEmpty
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              ...
                              style: TextStyle(fontSize: 12, color: AppTheme.outline, fontFamily: 'monospace'),
                            ),
                          ],
                        ),
                      )
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          final cert = _filteredCredentials[index];
                          return _GraduateCertificateCard(
                            certificate: cert,
                            ...
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => GraduateCredentialDetailScreen(
                                    credentialId: cert['credential_id'],
                                  ),
                                ),
                              );
...
class _GraduateCertificateCard extends StatelessWidget {
  final Map<String, dynamic> certificate;
  final String studentId;
  final VoidCallback onTap;
  
  const _GraduateCertificateCard({
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              height: 50,
              decoration: BoxDecoration(
                color: (isActive ? AppTheme.secondary : AppTheme.error).withOpacity(0.1),
                borderRadius: BorderRadius.circular(12),
              ),
              child: Icon(
                isActive ? Icons.verified : Icons.cancel,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    certificate['credential_id'] ?? 'Unknown',
                    style: const TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w500,
                      fontFamily: 'monospace',
                  ...
                  Row(
                    children: [
                      Icon(Icons.calendar_today, size: 12, color: AppTheme.outline),
                      const SizedBox(width: 4),
                      Text(
